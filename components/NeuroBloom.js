...
// Función auxiliar para convertir iconos en emojis estéticos
                    ...
                    <div className="animate-fadeIn max-w-6xl mx-auto space-y-12 relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-6">
                            {indTab === 'info' && appData.indicadores?.informacion?.map(item => (
                                <div key={item.id} onClick={() => setSubView(item.id)} className="cursor-pointer bg-white/80 backdrop-blur-xl p-8 rounded-[2rem] shadow-glass border border-white hover:shadow-xl transition-all transform hover:-translate-y-1 flex flex-col h-full group premium-card-border">
                                    <div className="text-5xl mb-4 group-hover:scale-110 transition-transform origin-left drop-shadow-md">
    ...
    return (
        <div className="min-h-screen font-sans relative overflow-hidden text-stone-700 bg-[#faf8f9]">
            ...
            <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
                ...
                <div className="absolute top-[-5%] left-[-5%] w-[25rem] h-[25rem] bg-brand-lilac/30 rounded-[40%_60%_70%_30%/40%_50%_60%_50%] animate-blob filter blur-md"></div>
                <div className="absolute top-[30%] right-[-5%] w-[20rem] h-[20rem] bg-yellow-400/20 rounded-[60%_40%_30%_70%/60%_30%_70%_40%] animate-blob animation-delay-2000 filter blur-md"></div>
                <div className="absolute bottom-[0%] left-[15%] w-[30rem] h-[30rem] bg-brand-green/20 rounded-[50%_50%_40%_60%/40%_60%_50%_40%] animate-blob animation-delay-4000 filter blur-md"></div>
                
                {/* Pinceladas/burbujas extra para llenar la página sin estorbar */}
                <div className="absolute top-[15%] left-[60%] w-12 h-12 bg-pink-400/30 rounded-full animate-bounce" style={{animationDuration: '4s'}}></div>
                <div className="absolute bottom-[20%] right-[20%] w-8 h-8 bg-brand-blue/30 rounded-full animate-bounce" style={{animationDuration: '5s'}}></div>
                <div className="absolute top-[40%] left-[10%] w-6 h-6 bg-brand-purple/30 rounded-full animate-pulse"></div>
            </div>
            ...
            <footer className="py-16 mt-12 bg-stone-900/95 backdrop-blur-md text-stone-400 flex flex-col items-center justify-center text-center relative z-10">
                ...
                <div className="flex gap-4 mb-6 relative z-20">
                    <a 
                        href="https://www.instagram.com/neuro_bloom.ags?igsh=MW9samc5azY3dnJ4Ng==" 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 border border-white/10 hover:bg-brand-purple hover:text-white hover:border-brand-purple transition-all duration-300 transform hover:-translate-y-1 shadow-sm"
                        aria-label="Instagram de Neuro Bloom"
                    >
                        <Instagram size={20} />
                        <span className="font-medium text-sm">@neuro_bloom.ags</span>
                    </a>
                </div>
                {/* ------------------------------- */}
